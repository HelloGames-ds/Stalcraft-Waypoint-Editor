...
Этот проект ориентирован на локальную работу с картой: редактирование `waypoints.cfg`, организация маркеров и размещение форм вейпоинтов по референсным изображениям поверх наборов зон в формате PNG. Репозиторий также включает workflow GitHub Actions для сборки автономного `.exe` под Windows.
